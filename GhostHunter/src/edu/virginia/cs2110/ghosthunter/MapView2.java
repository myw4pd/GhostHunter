...
public class MapView2 extends View implements LocationListener {
	...
	// the background.
	public MapView2(Context context, ArrayList<Ghost> ghosts, ArrayList<MoneyBag> money, int x, int y) {
		super(context);
		setFocusable(true);
	...
	public void onDraw(Canvas canvas) {
		...
		for (Ghost g : ghosts) {
			...
			canvas.drawCircle((float) g.getBone().getBoneX(), (float) g.getBone().getBoneY(), 40, paint);

		}

		for(MoneyBag m: money) {
			paint.setColor(Color.BLUE);
			canvas.drawCircle((float) m.getX(), (float) m.getY(), 40, paint);
		}
		...
		paint.setTextSize(100);
		canvas.drawText("Level " + level, 5, 300, paint);
		...
		if (level == 5) {
			...
			canvas.drawText("YOU WIN", 5, 600, paint);
		...
		for (final Ghost g : ghosts) {
			for(Bomb b : bombs) {
							...
							if (dialogup == false) {
								...
								dialogButton.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
										...
										ghosts.remove(g);
										Random rand = new Random();
										if(ghosts.size() == 0 && level < 4) {

											numghosts = numghosts *3/2;

											for (int i = 0; i < numghosts; i++) {

												int x = rand.nextInt((int) WIDTH);
												int y = rand.nextInt((int) HEIGHT);
				...
				public void stopUsingGPS() {
					if (locationManager != null) {
						locationManager.removeUpdates(MapView2.this);
					}
				}
